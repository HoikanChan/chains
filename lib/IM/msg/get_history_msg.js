...
//获取当前最后一条消息
function getNewMsg(selType,selToID,callback){
    if (selType == webim.SESSION_TYPE.GROUP) {
        //alert('当前的聊天类型为群聊天，不能进行拉取好友历史消息操作');
        return;
    }
    ...
    webim.getC2CHistoryMsgs(
        ...
        function(resp) {
            ...
            addsessMsg(resp.MsgList[0],selToID);
        }
    );
...
//获取最新的群历史消息,用于切换群组聊天时，重新拉取群组的聊天消息
var getLastGroupNewMsgs = function(cbOk) {
    if (selType == webim.SESSION_TYPE.C2C) {
        alert('当前的聊天类型为好友聊天，不能进行拉取群历史消息操作');
        return;
    }
    getGroupInfo(selToID, function(resp) {
        ...
        webim.syncGroupMsgs(
            ...
            function(msgList) {
                ...
                var msgSeq = msgList[0].seq - 1;
                // console.log(msgList[0].elems[0]);
                // addsessMsg(msgList[0].elems[0].content,selToID);
                // getPrePageGroupHistroyMsgInfoMap[selToID] = {
                //     "ReqMsgSeq": msgSeq
                // };
                //清空聊天界面
