...
function onSendMsg(msg) {
    ...
    //获取消息内容
    if(!msg){
        var msgContent = $("#send_msg_text").text();
    }else{
        var msgContent = msg;
    }
...
function handleMsgSend(msgContent) {
    ...
    webim.sendMsg(msg, function(resp) {
        ...
        addSess(selType,selToID);
        if(selType == webim.SESSION_TYPE.C2C){
            addsessMsg(msg,selToID);
        }
    }, function(err) {
        //alert(err.ErrorInfo);
        //提示重发
...
var showEmotionDialog = function() {
    ...
    if (emotionFlag) {
        ...
        $('#expression-model').fadeToggle(200);
        return;
    }
    emotionFlag = true;
...
var selectEmotionImg = function(selImg) {
    var txt = $('#send_msg_text');
    txt.value = txt.text() + selImg.id;
    txt.focus();
};