...
function addMsg(msg, prepend) {
    ...
    if (isSelfSend) { //如果是自己发的消息
        ...
        if (info && info.image) {
            ...
            if(msg.sess._impl.type == 'GROUP'){
                ...
                var groupList = db.get('group.userList.'+selToID).value();
                if(groupList){
                    var friendinfo = groupList[msg.fromAccount];
                    if(friendinfo){
                        fromAccountImage = (friendinfo.userSex ==  '女')?'../assets/images/6.png':'../assets/images/7.png';
                    }
                }
            }else{
    ...
    switch (subType) {
        ...
        case webim.GROUP_MSG_SUB_TYPE.TIP: //群提示消息
            $p1.html("[群提示消息]" + convertMsgtoHtml(msg));
            break;
    }

    $div1.append($div2.append($img1).append($p1));
...
function addSessfristMsg(msg,selToID)
{
    ...
    if(sessDiv){
        ...
        $p1.html(msg.MsgShow);
        var time = msg.MsgTimeStamp.substr(11,5);
        $time_span.text(time);
    }
