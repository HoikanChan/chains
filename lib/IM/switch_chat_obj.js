...
function SetGroupInfo(to_id){
    ...
    getGroupInfo(to_id,function(res){
        ...
        $('.app-chat-right-group-info-head').find('img').attr('src',FaceUrl);
        if(info.Name > 8){
            info.Name = info.Name.substr(0, 8) + "..."
        }
        $('.app-chat-right-group-name').text(info.Name);
        var userList = [];
        $.each(info.MemberList,(i,item)=>{
            if(item.Role == 'Owner' && item.Member_Account == loginInfo.identifier){
                ...
                $('#app-chat-group-quit').find('span').text('解散该群');
            }else{
                $('#app-chat-group-quit').attr({
                    'title':'退出该群',
                    'data-status':"1"
                });
                $('#app-chat-group-quit').find('span').text('退出该群');
            }
