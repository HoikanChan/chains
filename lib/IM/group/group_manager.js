...
var getGroupPublicInfo = function(GroupId) {
    if(!GroupId){
        GroupId = $("#sg_group_id").val();
    }
    if (GroupId.length == 0) {
        alert('请输入群组ID');
        return;
    }
    if (webim.Tool.trimStr(GroupId).length == 0) {
...
//创建群组
var createGroup = function(callback,err) {
    ...
    if(callback){
        webim.createGroup(options,callback,err);
        return;
    }
    webim.createGroup(
        options,
...
//获取我的群组
var getMyGroup = function(callback) {
    //initGetMyGroupTable([]);
    var options = {
    ...
    if(callback){
        webim.getJoinedGroupListHigh(options,callback);
        return;
    }
...
//获取我的好友，然后邀请加群
var getMyFriendGroup = function(callback) {
    ...
    if(callback){
        webim.getAllFriend(options,callback);
        return;
    }
