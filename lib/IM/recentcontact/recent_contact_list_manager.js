            ...
            if (resp.SessionItem && resp.SessionItem.length > 0) { //如果存在最近会话记录
                ...
                for (var i in resp.SessionItem) {
                    ...
                    if (type == webim.RECENT_CONTACT_TYPE.C2C) { //私聊
                        ...
                        if (c2cInfo && c2cInfo.image) { //从infoMap获取c2c头像
                            sessionImage = FileDoMain + c2cInfo.image; //会话头像，私聊时为好友头像，接口暂不支持返回，需要业务自己获取（前提是用户设置过自己的昵称，通过拉取好友资料接口（支持批量拉取）得到）
                        } else { //没有找到或者没有设置过
                            sessionImage = friendHeadUrl; //会话头像，如果为空，默认将其设置demo自带的头像
                        }
                        senderId = senderNick = ''; //私聊时，这些字段用不到，直接设置为空
                        ...
                        if (item.GroupImage) { //优先考虑接口返回的群头像
                            sessionImage = FileDoMain + item.GroupImage; //会话头像，群聊时，群头像，如果业务设置过群头像（设置群头像请参考wiki文档-设置群资料接口），接口会返回
                        } else { //接口没有返回或者没有设置过群头像，再从infoMap获取群头像
                            var key = sessType + "_" + sessionId;
                            var groupInfo = infoMap[key];
                            if (groupInfo && groupInfo.image) { //
                                sessionImage = FileDoMain + groupInfo.image
                            } else { //不存在或者没有设置过，则使用默认头像
                                sessionImage = groupHeadUrl; //会话头像，如果没有设置过群头像，默认将其设置demo自带的头像
                            }
                        }
                    ...
                    if (!tempSess) { //先判断是否存在（用于去重），不存在增加一个
                        ...
                        if(sessType == webim.SESSION_TYPE.GROUP){
                            getGroupInfo(sessionId,function(res){
                                if(res.GroupInfo[0].Type != 'ChatRoom'){
                                    var FaceUrl = (res.GroupInfo[0].FaceUrl)?fileDomain + res.GroupInfo[0].FaceUrl:'../assets/images/group.jpg';
                                    addSess(webim.SESSION_TYPE.GROUP, webim.Tool.formatText2Html(res.GroupInfo[0].GroupId), webim.Tool.formatText2Html(res.GroupInfo[0].Name), FaceUrl, 0);
                                    addSessfristMsg(recentSessMap[webim.SESSION_TYPE.GROUP + "_" + res.GroupInfo[0].GroupId],webim.Tool.formatText2Html(res.GroupInfo[0].GroupId));
                                }else{
                                    chatType = true;
                                }
                            });
                            ...
                            addSess(sessType, webim.Tool.formatText2Html(sessionId), webim.Tool.formatText2Html(sessionNick), sessionImage, item.UnreadMsgCount);
                            addSessfristMsg(recentSessMap[sessType + "_" + sessionId],webim.Tool.formatText2Html(sessionId));
                        }
                    }
