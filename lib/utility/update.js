...
$('#dw_setmin_btn').click(function(){
  ipcRenderer.send('download-min');
});

$('#dw_close_btn').click(function(){
  layer.confirm('新版本下载中,确定取消下载?', {
    btn: ['确定','取消'] //按钮
  }, function(){
...
function downloadFileCallback(arg, percentage)
{
    ...
    {
      layer.confirm('新版本下载完成，是否现在更新？', {
        btn: ['确定','取消'] //按钮
      }, function(){
        exec(percentage.path,function (e) {
          if(e){
            ipcRenderer.send('download-quit');
          }else{
            ipcRenderer.send('all-quit');
          }
        });
        setTimeout(function(){
          ipcRenderer.send('all-quit');
...
ipcRenderer.on('download-msg',(event,res)=>{
